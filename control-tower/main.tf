...
resource "aws_controltower_landing_zone" "this" {
  manifest_json = templatefile("${path.module}/landing-zone-manifest-4.0.json", {
    backup_admin_account_id         = data.terraform_remote_state.control_tower_prereqs.outputs.backup_admin_account_id
    backup_kms_key_arn              = aws_kms_key.backup.arn
    central_backup_account_id       = data.terraform_remote_state.control_tower_prereqs.outputs.central_backup_account_id
    centralized_logging_account_id  = data.terraform_remote_state.control_tower_prereqs.outputs.centralized_logging_account_id
    centralized_logging_kms_key_arn = aws_kms_key.centralized_logging.arn
    config_account_id               = data.terraform_remote_state.control_tower_prereqs.outputs.audit_account_id
    config_kms_key_arn              = aws_kms_key.config.arn
    security_roles_account_id       = data.terraform_remote_state.control_tower_prereqs.outputs.audit_account_id
